...
# A ``Co-authored-by: Display Name <email>`` trailer line. Case-insensitive,
# whole line, capturing the display name (everything before the ``<email>``).
# A squash-merge appends one such trailer per co-author, so this is the only
# offline signal for the humans a squash collapsed out of the single commit
# author, most importantly the source-PR authors of a squash-merged backport
# sweep whose sole commit author is the bot.
_COAUTHOR_RE = re.compile(r"(?im)^[ \t]*Co-authored-by:[ \t]*(.+?)[ \t]*<[^>]*>[ \t]*$")
...
def _coauthors_in_range(base_ref: str, head_ref: str, repo_dir: str) -> List[str]:
    ...
    Reads every commit body over the range with ``git log`` (offline, no token)
    and collects the display name of each ``Co-authored-by`` trailer, deduplicated
    ...
    try:
        out = subprocess.run(
            ...
            ["git", "log", "--reverse", "--format=%B", "{}..{}".format(base_ref, head_ref)],
            cwd=repo_dir,
            check=True,
            capture_output=True,
            text=True,
    ...
    names: List[str] = []
    seen = set()
    for match in _COAUTHOR_RE.finditer(out):
